...
def gram_matrix(features, normalize=True):
    ...
    features = features.reshape((N, C, M))
    
    gram = torch.sum( features[:,:,None,:] * features[:,None,:,:], axis=3 )
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for enum, l in enumerate(style_layers): 
      
      gram_currents = gram_matrix(feats[l])
      
      style_loss += style_weights[enum] * torch.sum(  torch.square( gram_currents - style_targets[enum] ),  axis=(1,2)  )
...
def tv_loss(img, tv_weight):
    ...
    vertical_delta = img[0, :, :-1, :] - img[0, :, 1:, :]

    horizontal_delta = img[0, :, :, :-1] - img[0, :, :, 1:]

    tv_loss = tv_weight * ( torch.sum(torch.square(vertical_delta)) + torch.sum(torch.square(horizontal_delta)) )
